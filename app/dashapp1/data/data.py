...
class Data(object):
    def get_data_confirmed(self):
        ...
        df_pivot['HasConfirmed']= np.where((df_pivot['CountConfirmed']>0), 1, 0)
        df_pivot['Seq'] = (df_pivot.sort_values(by=['Country', 'State', 'Date'], ascending=True)
                    .groupby(['Country', 'State'])['HasConfirmed'].cumsum())
